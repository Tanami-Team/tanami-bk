...
@endphp
@section('title',__('lang.user-verifications'))
@section('header')
    <!--begin::Heading-->
    <h1 class="text-dark fw-bolder my-0 fs-2">{{trans('lang.user-verifications')}} </h1>
    <!--end::Heading-->
    <!--begin::Breadcrumb-->
...
{{--                            <th class="min-w-50px">{{__('lang.type')}}</th>--}}
                            <th class="min-w-50px">{{__('lang.phone')}}</th>
                            <th class="min-w-125px">{{__('lang.UserName')}}</th>
                            <th class="min-w-50px">{{__('lang.Country')}}</th>
                            <th class="min-w-50px">{{__('lang.category')}}</th>
                            <th class="min-w-50px">{{__('lang.Users_active')}}</th>
...
@section('script')
    ...
    <script type="text/javascript">
        $(function () {
            var table = $('#users_table').DataTable({
                ...
                columns: [
                    ...
                    {data: 'user_name', name: 'user_name', "searchable": false, "orderable": true},
        ...
        function update_approved(el, route) {
            if (el.checked) {
                var is_approved = '1';
            } else {
                var is_approved = '0';
            ...
            $.post(route, {
                ...
                if (data == 1) {
                    toastr.success("<?php echo e(trans('lang.status_changed')); ?>");
                } else {
                    toastr.error("<?php echo e(trans('lang.status_changed')); ?>");
                }
            });
        }
    </script>
