...
<div class="fv-row mb-7">
    ...
    >
        ...
        <option value="{{$item->id}}" @if(isset($data)) @if($data->event_id == $item->id) selected @endif @endif>{{$item->name}}</option>
        @endforeach
    </select>
    <!--end::Input-->
</div>
...
<div class="fv-row mb-7">
    <div
        ...
        <label class="form-check-label" for="flexSwitchDefault">{{__('lang.is_notify')}}
            ؟</label>
        <input class="form-check-input" name="is_notify" type="hidden"
               value="inactive" id="flexSwitchDefault"/>
        <input class="form-check-input form-control form-control-solid mb-3 mb-lg-0"
               name="is_notify" type="checkbox"
               value="active" @if(isset($data)) @if($data->is_notify == 'active') checked @endif @endif id="flexSwitchDefault" />
    </div>
</div>
